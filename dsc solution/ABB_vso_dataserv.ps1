Configuration ABB_vso_dataserv {
    param(
    ...
    function Format-DscScriptBlock()
    {
        ...
        switch($appname){
            ...
            "toadoracle" {
                $toad126 = Get-AutomationVariable -Name "url_toad126"
                $result = $result.Replace("`$appurl", $toad126)
                    }
            "mapdrives" {
    ...
    node $nodename {    
        ...
        Script installtort
        {
            ...
            GetScript = { @{ Result = "placeholder"} }          
        }            
        Script installtoad
        {
            SetScript = Format-DscScriptBlock -Node $Node -appname "toadoracle" -ScriptBlock { 
                if ($pshome -like "*syswow64*") {
                    #write-warning "Restarting script under 64 bit powershell" 
                    # relaunch this script under 64 bit shell
                    # if you want powershell 2.0, add -version 2 *before* -file parameter
                ...
                (New-Object System.Net.WebClient).DownloadFile("$appurl", 'D:\ToadforOracle126SetupFull64.msi');
                $process = Start-Process -FilePath "D:\ToadforOracle126SetupFull64.msi" -ArgumentList '/q' -Wait -PassThru
            }
            TestScript = { 
                $appexist = Get-WmiObject -Class Win32_Product -filter "name='Toad for Oracle 12.6' AND version='12.6.0.53'"
                -not ($appexist -eq $null)
            }
            GetScript = { @{ Result = "12.6.0.53"} }          
        }
        package jrepkg
        {
